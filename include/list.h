 ...
 *     - 描述：复制整个链表，生成一个新的链表，其中节点数据为原链表数据（深拷贝）。
 *     - 返回：新创建的链表。
 *
...
#define List(Type)                                                                                                            \
    ...
    {                                                                                                                         \
        Type##Node *current = list->head;                                                                                     \
        while (current)                                                                                                       \
        {                                                                                                                     \
            Type##Node *temp = current->next;                                                                                 \
            free(current);                                                                                                    \
            current = temp;                                                                                                   \
        }                                                                                                                     \
        list->head = NULL;                                                                                                    \
    ...
    /*排序链表(归并排序)*/                                                                                                    \
    static inline Type##Node *Type##List##_Merge(Type##Node *left, Type##Node *right, int (*cmp)(const Type *, const Type *)) \
    {                                                                                                                         \
        if (!left)                                                                                                            \
            return right;                                                                                                     \
        if (!right)                                                                                                           \
        ...
        {                                                                                                                     \
            left->next = Type##List##_Merge(left->next, right, cmp);                                                          \
            return left;                                                                                                      \
        }                                                                                                                     \
        else                                                                                                                  \
        {                                                                                                                     \
            right->next = Type##List##_Merge(left, right->next, cmp);                                                         \
            return right;                                                                                                     \
        }                                                                                                                     \
    }                                                                                                                         \
    static inline Type##Node *Type##List##_MergeSort(Type##Node *head, int size, int (*cmp)(const Type *, const Type *))      \
    {                                                                                                                         \
        if (size < 2)                                                                                                         \
            return head;                                                                                                      \
        int mid = size / 2;                                                                                                   \
        ...
        for (int i = 1; i < mid; i++)                                                                                         \
            curr = curr->next;                                                                                                \
        Type##Node *second = curr->next;                                                                                      \
        curr->next = NULL;                                                                                                    \
        Type##Node *left = Type##List##_MergeSort(head, mid, cmp);                                                            \
        Type##Node *right = Type##List##_MergeSort(second, size - mid, cmp);                                                  \
        return Type##List##_Merge(left, right, cmp);                                                                          \
    }                                                                                                                         \
    static inline void Type##List##_Sort(Type##List list, int (*cmp)(const Type *, const Type *))                             \
    {                                                                                                                         \
        if (!list || list->size < 2)                                                                                          \
            return;                                                                                                           \
        list->head = Type##List##_MergeSort(list->head, list->size, cmp);                                                     \
        /* 更新tail指针 */                                                                                                    \
        Type##Node *cur = list->head;                                                                                         \
        while (cur && cur->next)                                                                                              \
            cur = cur->next;                                                                                                  \
    ...
    /*复制链表（浅拷贝）*/                                                                                                    \
    static inline Type##List Type##List##_Copy_Shallow(Type##List list)                                                       \
    {                                                                                                                         \
        if (!list)                                                                                                            \
            return NULL;                                                                                                      \
        Type##List newList = (Type##List)malloc(sizeof(struct Type##List));                                                   \
        if (!newList)                                                                                                         \
            return NULL;                                                                                                      \
    ...
    /*复制链表（深拷贝）*/                                                                                                    \
    static Type##List Type##List##_Copy_Deep(Type##List list)                                                                 \
    {                                                                                                                         \
        if (!list)                                                                                                            \
            return NULL;                                                                                                      \
